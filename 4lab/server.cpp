...
/*
┌───┐
│ 7 │ Прикладной (Application) — HTTP, SMTP, FTP
└─┬─┘
  │ (Данные)
  ▼
┌───┐
│ 6 │ Представительский (Presentation) — JPEG, ASCII, Шифрование
└─┬─┘
  │ (Данные)
  ▼
┌───┐
│ 5 │ Сеансовый (Session) — Управление сессиями (RPC, PAP)
└─┬─┘
  │ (Данные)
  ▼
┌───┐
│ 4 │ Транспортный (Transport) — TCP, UDP (Порты)
└─┬─┘
  │ (Сегменты / Датаграммы)
  ▼
┌───┐
│ 3 │ Сетевой (Network) — IP-адреса, Маршрутизаторы
└─┬─┘
  │ (Пакеты)
  ▼
┌───┐
│ 2 │ Канальный (Data Link) — MAC-адреса, Свитчи
└─┬─┘
  │ (Кадры / Фреймы)
  ▼
┌───┐
│ 1 │ Физический (Physical) — Биты, Кабели, Wi-Fi
└───┘
  |_ (Электрические/Оптические сигналы)
*/
