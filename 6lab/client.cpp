    ...
    while (is_connected.load()) {
        ...
        for (const auto& it : items) {
            ...
            std::cout << "[Transport][RETRY] resend " << it.pm.retries << "/" << RETRY_MAX
                      << " (id=" << it.pm.msg_id << ")\n";
            (void)send_message_ex(sock, it.pm.type, it.pm.msg_id, it.pm.sender,
...
void run_ping(int sock, const std::string& nickname, int count) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        if (!send_message_ex(sock, MSG_PING, id, nickname, "",
...
int main(int argc, char* argv[]) {
    ...
    while (true) {
        ...
        (void)send_message_ex(sock, MSG_HELLO, 0, nickname, "", std::time(nullptr),
                              "My name is " + nickname);
        ...
        pthread_t retry_tid{};
        auto* re_args = new RetryThreadArgs{sock};
        ...
        while (is_connected.load()) {
            ...
            if (input == "/ping" || input.rfind("/ping ", 0) == 0) {
                ...
                run_ping(sock, nickname, ping_count);
                continue;
            }
            ...
            if (parse_whisper_cmd(input, target, msg_text)) {
                ...
                std::uint32_t id = next_msg_id();
                (void)send_reliable(sock, MSG_PRIVATE, id, nickname, target,
                                    std::time(nullptr), msg_text);
                continue;
            }

            std::uint32_t id = next_msg_id();
            (void)send_reliable(sock, MSG_TEXT, id, nickname, "",
