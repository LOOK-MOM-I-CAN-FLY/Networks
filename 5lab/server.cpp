...
void log_outgoing(int sock, std::uint8_t type, std::size_t payload_len, const std::string& action) {
    sockaddr_in local{};
    sockaddr_in peer{};
    (void)get_endpoints(sock, local, peer);
...
bool send_message_tcpip(
    ...
    log_outgoing(sock, type, payload_len, action);
    ...
    while (true) {
        ...
        while (true) {
            ...
            if (msg.type == MSG_PING) {
                (void)send_message_tcpip(client_sock, MSG_PONG, 0, "SERVER", "", std::time(nullptr), "", "send pong");
                continue;
            }
