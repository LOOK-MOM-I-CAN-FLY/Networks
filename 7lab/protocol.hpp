...
constexpr int SERVER_PORT_DEFAULT = 5555;
...
inline bool ssl_recv_all(SSL* ssl, void* data, std::size_t size) {
    ...
    while (size > 0) {
        ...
        ptr += static_cast<std::size_t>(got);
...
inline bool send_message_ex(
    ...
    if (!ssl_send_all(ssl, &header, sizeof(header))) {
        return false;
    }
    if (payload_len == 0) {
        return true;
...
inline bool recv_message_ex(SSL* ssl, MessageEx& msg) {
    ...
    MessageExWireHeader header{};
    if (!ssl_recv_all(ssl, &header, sizeof(header))) {
        return false;
    }
    ...
    if (payload_len > 0) {
        if (!ssl_recv_all(ssl, msg.payload, payload_len)) {
            return false;
        }
    }
...
inline void openssl_global_init() {
